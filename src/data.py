    ...
    def __getitem__(self, idx):
        ...
        if (self.inmodal):
            ...
            item["pixel_values"] = self.processor.process_image(
                Image.open(os.path.join(self.root, self.images[idx]))), self.processor.process_image(
...
def get_train_dataloader(options, processor):
    ...
    dataset = ImageCaptionDataset(path,
                                  image_key=options.image_key,
                                  caption_key=options.caption_key,
...
def get_eval_test_dataloader(options, processor):
    ...
    if (options.eval_data_type == "Caltech101"):
        ...
        dataset = torchvision.datasets.StanfordCars(root=os.path.dirname(options.eval_test_data_dir), download=True,
                                                    split="test", transform=processor.process_image)
